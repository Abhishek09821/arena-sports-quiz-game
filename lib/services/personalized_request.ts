...
export async function generateForRequest(req: Request, params: CreateQuizRequest) {
  ...
  if (!auth.userId || !client) throw new Error("Sign in again to prepare a round with your saved question history.");
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    for (let offset = 0; ; offset += 1000) {
      ...
      // History is required for authenticated cross-device duplicate protection.
      if (error) {
        console.warn("[question history] Read unavailable:", error.message);
        throw new Error("Question history is unavailable. Please retry shortly; your round has not started.");
      }
    ...
    if (error) {
      console.warn("[question history] Reservation unavailable:", error.message);
      throw new Error("Could not reserve fresh questions. Please retry shortly.");
    }
    if (data === true) return deck;
    // Another tab reserved one of these questions; reload history and replace the deck.
  }
  throw new Error("Another round reserved these questions. Please retry for a fresh round.");
}
