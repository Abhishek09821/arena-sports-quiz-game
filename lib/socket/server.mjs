...
async function persistRoomStatus(room, status) {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) return;
  try {
    const db = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, {auth:{persistSession:false}});
    const {error} = await db.from("game_rooms").update({status,updated_at:new Date().toISOString()}).eq("code",room.code);
    if (error) console.warn("Room status sync failed:", error.message);
  } catch { console.warn("Room status sync unavailable"); }
}
...
function finishGame(io, room) {
  ...
  void persistRoomStatus(room, "finished");
  room.phase = "revealed";
...
export function setupSocketServer(io) {
  io.on("connection", (socket) => {
    ...
    socket.on("join_room", (data, callback) => {
      try {
        const { roomCode, playerName = "Player 2", playerToken, seenStems = [] } = data || {};
        ...
        if (room.players[playerToken]) {
          ...
          if (playerName) room.players[playerToken].name = playerName.trim().slice(0, 24);
          room.players[playerToken].seenStems = Array.isArray(seenStems) ? seenStems.filter(s => typeof s === "string") : [];
        ...
        room.players[playerToken] = {
          ...
          seenStems: Array.isArray(seenStems) ? seenStems.filter(s => typeof s === "string") : [],
          score: 0,
          ready: true,
          connected: true,
        };
    ...
    socket.on("start_game", (data, callback) => {
      try {
        ...
        if (Array.isArray(data.questions) && data.questions.length === room.questions.length) {
        ...
        room.status = "playing";
        void persistRoomStatus(room, "live");
